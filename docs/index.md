...
* **Управление кнопками** — позволяет выбрать одну из схем взаимодействия с физическими кнопками подогрева:
  * **Стандартное:** всё управление осуществляется одиночными нажатиями по кругу. При обычном порядке уровней: «Уровень 1 -> Уровень 2 -> Уровень 3 -> Выкл». При инвертированном порядке: «Уровень 3 -> Уровень 2 -> Уровень 1 -> Выкл». В стандартном режиме используется схема «Вкл -> Выкл».
  * **Альтернативное:**
...
* **Порядок уровней** — задаёт порядок отображения и переключения уровней в приложении, системном и динамическом виджетах, а также на физических кнопках контроллера. Обычный порядок: «1 -> 2 -> 3». Инвертированный порядок: «3 -> 2 -> 1». Индикация всегда соответствует фактически выбранному уровню, без обратного пересчёта.
